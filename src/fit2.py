...
def estimate_edge_coefficients(U_prime, spectral_matrix):
    ...
    if U_prime.shape[1] != 2:
        raise ValueError(
            f"Per fit2 U_prime deve avere 2 colonne, trovate: {U_prime.shape[1]}"
        )
...
def run_fit2(config_path: str, run_metadata: dict):
    ...
    run_name = f"fit2_{run_metadata['run_timestamp'].replace(':', '-').replace(' ', '_')}"
    ...
    def fun1(x_free):
        ...
        return residuals_fit2(x_full, T, V_prime, pack)
